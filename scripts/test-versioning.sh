...
# Testing Script for MONIK-ENTERPRISE Versioning System
...
# Test 4: Check if pre-commit hook exists and is executable
test_precommit_hook() {
...
test_versioning_scripts() {
    if [ -f "scripts/version-bump.sh" ] && [ -f "scripts/migrate-changelog.sh" ]; then
        if [ -x "scripts/version-bump.sh" ] && [ -x "scripts/migrate-changelog.sh" ]; then
            return 0
        fi
    fi
    return 1
...
# Test 6: Test conventional commit message validation
test_commit_validation() {
    # Create a test commit message file
    local test_msg="feat(api): add new endpoint"
    echo "$test_msg" > /tmp/test_commit_msg
    
    # Test with valid commit message (should pass)
    if .git/hooks/pre-commit /tmp/test_commit_msg > /dev/null 2>&1; then
...
# Test 7: Test invalid commit message validation
test_invalid_commit_validation() {
    # Create a test commit message file
    local test_msg="invalid commit message"
    echo "$test_msg" > /tmp/test_commit_msg
...
# Test 8: Check if VERSIONING.md exists
...
# Test 9: Check environment variables in .env.example
test_env_variables() {
...
# Test 10: Test version bump script syntax
...
# Test 11: Test migrate changelog script syntax
...
# Test 12: Check if git repository is initialized
test_git_repository() {
...
main() {
    ...
    run_test "Commit validation (valid)" "test_commit_validation" "pass"
    run_test "Commit validation (invalid)" "test_invalid_commit_validation" "pass"
